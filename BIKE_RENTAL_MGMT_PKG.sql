...
CREATE OR REPLACE PACKAGE BODY bike_rental_mgmt_pkg AS
  ...
  --  PROCEDURE: Return bike to end dock
    ...
    BEGIN
        UPDATE docks
        SET
            bike_available = bike_available + 1
        WHERE
...
-- Fix: explicitly pass values as strings
        dbms_scheduler.set_job_argument_value(v_job_name,
                                              1,
                                              to_char(p_bike_id));
        dbms_scheduler.set_job_argument_value(v_job_name,
                                              2,
                                              to_char(p_end_dock_id));
    ...
    BEGIN
        INSERT INTO rental (
            ...
            end_date_time
                   ...
                   p_end_dock_id,
                   p_start_datetime,
                   p_end_datetime ) RETURNING rental_id INTO po_rental_id;

    -- Call availability update + schedule job
        DECLARE
            v_msg VARCHAR2(200);
        BEGIN
            v_msg := update_bike_availability(p_bike_id, p_start_dock_id, p_end_dock_id, p_start_datetime, p_end_datetime);
